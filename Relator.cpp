...
//=============================================================================
Cursor::RelativeBucket Relator::getParents(TNode *node) {
         return history[node->age].parents;
};

//---------------------------------------------------------------------------

void Relator::calculateParents(TNode *node) {
    RelativeBucket &parents = getParents(node);
    parents.count = 0;
    for (int i = 0; i < TOTAL_CELLS; ++i) {
        if (kl[i]==1) {
            ...
            unsigned long multiplier = simplyGen->getHash(i);//TODO get only existing hash, so new simply number cab't be generated by this call
